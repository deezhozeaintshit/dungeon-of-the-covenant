...
def normalize_model(imported):
    ...
    # World-space bounds of the CHARACTER meshes (root is still identity here).
    # NOTE: Blender's glTF importer spawns a helper 'Icosphere' mesh when a
    # skin is present — exclude it by measuring only rig-associated meshes
    # when an armature was imported.
    armatures = set(o for o in imported if o.type == 'ARMATURE')

    def _is_character_mesh(o):
        if o.type != 'MESH':
            return False
        if not armatures:
            return True
        if o.parent in armatures:
            return True
        return any(m.type == 'ARMATURE' and m.object in armatures for m in o.modifiers)

    mn = [float("inf")] * 3
    mx = [float("-inf")] * 3
    found = False
    for o in imported:
        if not _is_character_mesh(o):
            continue
...
def render_views(cam, outdir, name):
    ...
    dist, cam_z = 4.4, 1.15
    import os as _os
    skip = _os.environ.get("SKIP_VIEWS", "").split(",")
    views = [
        ("front", 0),
        ("threequarter", 45),
        ("side", 90),
    ...
    views = [(n, d) for n, d in views if n not in skip]
