...
def import_glb(path):
    ...
    # Blender's glTF importer synthesizes an "Icosphere" mesh and assigns it
    # as a custom bone shape on skinned imports. It is NOT in the source file:
    # strip it (and clear the bone references) so framing/rendering is clean.
    helpers = set()
    for o in imported:
        if o.type == "ARMATURE":
            for b in o.pose.bones:
                if b.custom_shape is not None:
                    helpers.add(b.custom_shape)
                    b.custom_shape = None
    for o in list(helpers):
        if o.name in bpy.data.objects:
            bpy.data.objects.remove(o, do_unlink=True)
    imported = [o for o in imported if o not in helpers]
    log(f"imported {len(imported)} objects (removed {len(helpers)} bone-shape helpers)")
    return imported
